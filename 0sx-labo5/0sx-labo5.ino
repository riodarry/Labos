...
void synchroniserPositionMoteur() {
  positionVanne = moteur.currentPosition();
}
...
void clicBouton() {
  ...
  } else if (etatLCD == DHT_STATE) {
    changerEtatLCD(LUM_DIST_STATE);
  } else if (etatLCD == CALIB_STATE) {
    changerEtatLCD(LUM_DIST_STATE);
  } else if (etatLCD == VANNE_STATE) {
    changerEtatLCD(LUM_DIST_STATE);
  }
}
...
void bootState(unsigned long ct) {
  ...
  if (ct - previousTime >= rate) {
    previousTime = ct;
    afficherTexteFixe("Demarrage...", "Maison_02");
  }
...
void lumiereTache(unsigned long ct) {
  ...
  if (lumiereMax > lumiereMin) {
    lumierePct = map(lumiereBrute, lumiereMin, lumiereMax, pctMin, pctMax);
    lumierePct = (lumierePct, 0, 100);
  }
}
...
void envoieSerieTache(unsigned long ct) {
  ...
  Serial.print(",Van:");
  Serial.println(pourcentageVanne());
}
...
int pourcentageVanne() {
  unsigned long pct = map(positionVanne, VANNE_FERMEE, VANNE_OUVERTE, pctMin, pctMax);
  return pct;
}
